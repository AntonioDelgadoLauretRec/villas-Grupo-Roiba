...
export default function VillasContent({ villas }: { villas: Villa[] }) {
        ...
        <div className="max-w-7xl mx-auto">
          {villas.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-roiba-verde/60 text-body-lg">{tx.proximamente}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {villas.map((villa) => (
                <Link key={villa.id} href={`/villas/${villa.slug}`} className="group block">
                  ...
                  <div>
                    ...
                    {villa.short_description && (
                      <p className="text-body text-roiba-verde/60 mt-2 line-clamp-2">{villa.short_description}</p>
                    )}
                    {(villa.bedrooms > 0 || villa.area_sqm > 0) && (
                      <div className="flex items-center gap-4 mt-3 text-caption text-roiba-verde/50">
                        {villa.bedrooms > 0 && <span>{villa.bedrooms} {tx.hab}</span>}
                        {villa.bathrooms > 0 && <span>{villa.bathrooms} {tx.banos}</span>}
