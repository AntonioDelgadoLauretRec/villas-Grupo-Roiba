...
export default function BrochureSection() {
  ...
  return (
    <section className="py-[clamp(60px,8vw,100px)] px-[clamp(24px,8vw,120px)] bg-roiba-arena-light relative overflow-hidden">
      ...
      <div className="max-w-[900px] mx-auto relative z-[2] text-center">
        ...
        {status === 'success' ? (
          <div className="scroll-reveal delay-3 inline-flex items-center gap-2 px-6 py-3 bg-green-50 text-green-700 rounded-sm font-sans text-sm font-medium">
            ...
            ¡Gracias! Revise su correo electrónico.
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="scroll-reveal delay-3 flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            ...
            >
              {status === 'loading' ? 'Enviando...' : 'Descargar guía'}
            </button>
          </form>
        )}

        {status === 'error' && (
          <p className="mt-3 text-red-500 text-sm font-sans">Error al enviar. Inténtelo de nuevo.</p>
        )}
      </div>
    </section>
  )
